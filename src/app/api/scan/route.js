...
const MAX_HOSTS = parseInt(process.env.MAX_HOSTS || "256", 10); // keep requests small in a single HTTP call
const REQ_BUDGET_MS = parseInt(process.env.REQ_BUDGET_MS || "55000", 10); // leave headroom for response

const hostCount = estimateHosts(target);
...
export async function GET(req) {
    ...
    try {
        ...
        // limits
        const MAX_HOSTS = Number(process.env.MAX_HOSTS || 4096);
        const hostCount = estimateHosts(target);
        if (hostCount > MAX_HOSTS) {
            return Response.json(
                {
        ...
        const estimated = estimateHostCount(target);
        const discoveryTimeoutMs = Math.max(15_000, Math.min(REQ_BUDGET_MS - 5_000, Math.floor(hostCount * 120)));
